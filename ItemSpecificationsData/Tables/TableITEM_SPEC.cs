...
{
    public class TableITEM_SPEC : CDBTable<ITEM_SPEC>
    {
        public int MasterID { get; set; }

        // --------------------------------------------------------------------------------------
        public TableITEM_SPEC()
        {
        }
        // --------------------------------------------------------------------------------------
        ...
        public override void LoadTable(IDbTransaction? p_iTransaction)
        {
            ...
            ITEM_SPEC? oParams = new ITEM_SPEC();
            oParams.ITEM_ID = this.MasterID;
            var oRecords = CData.Instance.DB.SelectWithParams<ITEM_SPEC>(
                    "select * from ITEM_SPEC where ITEM_ID = @ITEM_ID", oParams, p_iTransaction);
        ...
        public override void SaveTable(IDbTransaction? p_iTransaction)
        {
            ...
            {
                CData.Instance.DB.SaveChanges<ITEM_SPEC>(this.records,

                            // Provide the insert statement that will be used for new records
                            @"  insert into ITEM_SPEC
                                    (ITEM_ID, SPEC_TYPE_CID, TITLE, VALUE)
                                    values
                                    (@ITEM_ID, @SPEC_TYPE_CID, @TITLE, @VALUE)",
